...
const int buttled =  24; 
const int button =  22; 
const int intellivue = 14;
const int propaq = 15;
const int batt = 16;
...
//
#elif RAMEND < 0X40FF
// Use total of 13 512 byte buffers. (auf 8 per Hand reduziert)
const uint8_t BUFFER_BLOCK_COUNT = 8;
// Dimension for queues of 512 byte SD blocks.
const uint8_t QUEUE_DIM = 16;  // Must be a power of two!
//
#else  // RAMEND
// Use total of 29 512 byte buffers. (auf 8 per Hand reduziert)
const uint8_t BUFFER_BLOCK_COUNT = 8;
// Dimension for queues of 512 byte SD blocks.
...
void binaryToCsv() {
  ...
  csvStream.println(F(",usec"));
   csvStream.print(month(jetzt));
   csvStream.print("-");
   csvStream.print(day(jetzt));
   csvStream.print("-");
   csvStream.print(year(jetzt));
   csvStream.print("  ");
   csvStream.print(hour(jetzt));
   csvStream.print(":");
   csvStream.println(minute(jetzt));
...
void logData() {
  ...
  while (1) {
    ...
    else lcd.print(sek % 60);
    
    
    
    
    if (Serial.available() || (digitalRead(button)==LOW && (millis() - pushbutton > 5000) ) ) {
      // Stop ISR calls.
...
void setup(void) {
  ...
  Serial.begin(9600);
  pinMode(button, INPUT);     // Taster PIN = Eingang
  digitalWrite(button, HIGH);
  pinMode(intellivue, INPUT); 
  digitalWrite(intellivue, HIGH);
  pinMode(propaq, INPUT); 
  digitalWrite(propaq, HIGH);
  pinMode(batt, INPUT); 
  pinMode(buttled, OUTPUT); 
  setSyncProvider(RTC.get);
  //RTC.adjust(DateTime(__DATE__, __TIME__));
  
  lcd.createChar(0, fullbatt);
  lcd.createChar(1, lowbatt);
  lcd.createChar(2, emptybatt);
  lcd.begin(16,2);
  lcd.home();
  lcd.print("Starting...");
  lcd.setCursor(0,1);
  ...
  if (digitalRead(intellivue) == HIGH && digitalRead(propaq) == HIGH) {
   lcd.clear();
   lcd.home();
  ...
  if (digitalRead(intellivue) == LOW)
  {
   lcd.clear();
   lcd.home();
   lcd.print("Intellivue Cable");
   lcd.setCursor(0,1);
   lcd.print("detected!");
   delay(2000);
  }
  if (digitalRead(propaq) == LOW)
  {
   PIN_LIST[0] = 1;
   lcd.clear();
   lcd.home();
   lcd.print("Propaq Cable");
   lcd.setCursor(0,1);
   lcd.print("detected!");
   delay(2000);
  }
...
void loop(void) {
  ...
  if (digitalRead(button)== LOW && record == 1){
    binaryToCsv();
  }
